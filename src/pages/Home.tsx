  ...
  return (
    <main className="pt-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="animate-fade-up">
            ...
            <div className="flex flex-wrap gap-3">
              <Link to="/diseases" className="flex items-center gap-2 px-5 py-2.5 rounded-xl border-2 border-white text-white font-semibold text-sm hover:bg-white hover:text-red-600 transition-colors">
                <IconSearch size={16} /> Disease Library
              </Link>
              <a href="tel:907" className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-red-600 font-semibold text-sm hover:bg-red-50 transition-colors">
          ...
          <div className="hidden lg:block animate-fade-up delay-200">
            <div className="relative">
              <div className="absolute inset-0 rounded-2xl bg-red-500/30 translate-x-4 translate-y-4" />
              <img
                src="https://images.unsplash.com/photo-1559839734-2b71ea197ec2?w=600&h=480&fit=crop&auto=format"
                alt="Healthcare professional"
                className="relative rounded-2xl w-full h-[380px] object-cover border-4 border-red-500/30"
              />
            </div>
          </div>
        </div>
        ...
        <div ref={statsRef} className="max-w-4xl mx-auto px-4 sm:px-6 py-10 grid grid-cols-3 gap-6">
          {STATS.map(({ icon: Icon, value, label }) => (
            <div key={label} className="flex flex-col items-center text-center">
              <Icon size={28} className="text-red-600 mb-2" />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {WHY.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md hover:border-gray-300 transition-all duration-200">
                <Icon size={28} className="text-red-600 mb-4" />
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {FEATURED.map(({ icon: Icon, tag, title, desc, href }) => (
              <div key={title} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md hover:border-gray-300 transition-all duration-200 group">
                ...
                <Link to="/diseases" className="text-sm font-semibold text-red-600 hover:text-red-700 transition-colors flex items-center gap-1 group-hover:gap-2">
      ...
      <section ref={ctaRef} className="py-8 max-w-6xl mx-auto px-4 sm:px-6 flex flex-col gap-5">
        {/* Emergency CTA */}
        <div className="rounded-2xl bg-gradient-to-r from-red-600 to-rose-600 text-white p-10 text-center">
          <h3 className="font-display font-extrabold text-2xl sm:text-3xl mb-2">Need Immediate Help?</h3>
          <p className="text-red-100 text-sm mb-6">Get instant access to healthcare professionals and emergency services</p>
        ...
        {/* Health Tips CTA */}
        <div className="rounded-2xl bg-gradient-to-r from-rose-500 to-pink-600 text-white p-10 text-center">
          <h3 className="font-display font-extrabold text-2xl sm:text-3xl mb-2">Daily Health Tips</h3>
          <p className="text-red-100 text-sm mb-6">Get personalized health tips and wellness advice to improve your daily life</p>
          <Link to="/health-tips" className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-white text-red-600 font-semibold text-sm hover:bg-red-50 transition-colors">
            Explore Health Tips
          </Link>
        </div>
      </section>
